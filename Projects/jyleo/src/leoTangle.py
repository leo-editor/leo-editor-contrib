...
class baseTangleCommands:
    ...
    def tanglePass1(self,p):
        ...
        while p and not p == next: #CHANGED: p !=
            self.p = p
    ...
    def untangleRoot(self,root,begin,end):
        ...
        if self.errors == 0:
            ...
            p = begin
            while p and not p == end: # Don't use iterator. #CHANGED p !=
                self.tanglePass1(p)
                if self.errors != 0:
                    break
                p.moveToNodeAfterTree()
    ...
    def untangleTree(self,p,report_flag):
        ...
        while p and not p == afterEntireTree and self.errors == 0: #CHANGED p !=
            self.setRootFromHeadline(p)
            theDict = g.get_directives_dict(p.bodyString(),[self.head_root])
            ...
            if ignore:
                ...
                p.moveToThreadNext()
                while p and not p == afterUnit and self.errors == 0: #CHANGED: p !=
                    self.setRootFromHeadline(p)
                    theDict = g.get_directives_dict(p.bodyString(),[self.head_root])
                    root = theDict.has_key("root")
                    if root:
