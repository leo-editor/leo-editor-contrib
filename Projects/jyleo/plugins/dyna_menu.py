...
def dynaS_importnode(c):
  ...
  ~EOT    overwrite False, no sfdots or evaluator
    ...
    if overwrite: 
        ...
        body = p.c.frame.bodyCtrl
        doc = body.getDocument()
        body.setSelectionStart( 0 )
        body.setSelectionEnd( doc.getLength() )
...
def load_menu(tag, keywords ):
    ...
    rb1 = swing.JRadioButtonMenuItem( "print" )
    rb1.setSelected( 1 )
    rb1.actionPerformed = togprpa( cf = cf )
    group.add( rb1 )
    nu.add( rb1 )
    rb2 = swing.JRadioButtonMenuItem( "paste" )
    rb2.actionPerformed = togprpa( cf = cf )
    group.add( rb2 )
    nu.add( rb2 )
    rb3 = swing.JRadioButtonMenuItem( "doc" )
    rb3.actionPerformed = togprpa( cf = cf )
    group.add( rb3 )
    nu.add( rb3 )
    ...
    for x in 'lower upper reverse caps <base64  >base64 <rot13 >rot13 >hex <hex >md5'.split():
        ...
        jmi = swing.JMenuItem( x )
        jmi.actionPerformed = lambda event, x = x, c = c: dynax_actions( c, x )
        dynai.add( jmi )
    ...
    #dynai = Tk.Menu(None, tearoff= 0, takefocus= 0 )
    dynai = swing.JMenu( "slashs" )
    for x in  'Do bs, : file, | !file, \\\\ Dbs'.split(', '):
        #dynai.add_command(label= x, 
        #      command= lambda c= c, r= x.split()[0]:  dynax_slashs(c, r) ) 
        jmi = swing.JMenuItem( x )
        jmi.actionPerformed = lambda event, c = c, r = x.split()[ 0 ]: dynax_slashs( c, r )
        dynai.add( jmi )
